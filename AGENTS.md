...
- **Objetivo:** Calcular o Adicional de Qualificação (AQ) do Judiciário conforme legislação vigente
- **Stack:** HTML + CSS + JavaScript vanilla (sem build step de app)
- **OS Context:** Windows 11 (dev) · Cross-Platform
- **Documentação Local:** `README.md`, `PRD.md`, `CHANGELOG.md`
...
6. **Não espere o usuário lembrar que o achado foi resolvido.** No `Prompt 23` (sync) ou na abertura
   ...
   Hub (CHANGELOG, skills novas, commits recentes). Se o que motivou o achado já foi corrigido lá,
   feche sozinho (item 5) — sem esperar o usuário notar e pedir para verificar.

   **A ferramenta lista o que conferir, não conclui por você:** `hub_state`/`npm run state`, rodado no
   satélite, mostra os achados da própria fila legada em `nextActions` (dimensão `findings`, corrigido
   03/09/2026 — antes era pulada inteira quando `role !== "hub"`, e a regra acima não tinha como ser
   seguida na prática: achado real, oficina-uas, agente reportou achado "ainda aberto" que já tinha
   sido corrigido no Hub dias antes, porque a ferramenta não devolvia nada pra conferir — a mesma
   lacuna estrutural, achado invisível se o satélite não estivesse clonado localmente, motivou a
   migração pra GitHub Issues no item 4). O passo de cruzar cada achado listado contra o CHANGELOG e
   decidir se já foi corrigido continua manual em ambos os casos.
...
## 📁 Estrutura do Projeto (CalcAQ)

Este é um projeto **puro HTML/CSS/JavaScript** sem build step de app (sem React/Vite). A governança do Hub adiciona tooling Node só para scripts de qualidade/hooks.
...
**⚠️ IMPORTANTE:** Sempre consulte `PRD.md` (e `ANALISE_INCONSISTENCIAS.md` se existir) antes de modificar cálculos ou regras de negócio.

---

## 🛠️ Tecnologias e Padrões (produto)
...
- Variáveis CSS; mobile-first; Grid/Flexbox; sem frameworks externos
...
3. 2 Pós + 1 Grad + 2 Cert → 2 VR (Pós + Grad; Cert descartadas)
4. Capacitações acumulam com tudo (inclusive Doutorado)
